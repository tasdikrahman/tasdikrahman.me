...
concept when being used, but when you try to get your head around the underlying details, you find yourself in a hot fix.
...
If have been dabbling away in python, this might not seem out of the blue I assume.
...
>>> 
```
Not that surprising then I suppose. The only gotcha here would be that the inner enclosing functions have the access to the
...
Taling about closures, this can be used very cleverly in some cases 
...
You can see a special behavious here demonstrated by the function `foo()`. It remembers the value passed to it between function
